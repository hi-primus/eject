...
class DuckDBClient {
    ...
    // https://shell.duckdb.org/docs/interfaces/index.CSVInsertOptions.html
    ...
    async insertCSVURL(name, url, options) {
        const db = await this.db();
        await db.registerFileURL(name, url,'HTTPS');
        console.log("url",url)
        const conn = await db.connect();
        // await conn.insertCSVFromPath(name,{name});
        await conn.insertCSVFromPath(url, { name:name, ...options });
        await conn.close();
